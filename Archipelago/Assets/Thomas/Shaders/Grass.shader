    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float, _Metal)
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float4 node_5748 = _Time;
                float node_2160 = (_Intensity_var*(sin(mul(unity_ObjectToWorld, v.vertex).r)*((1.0 - o.uv0.g)*cos(node_5748.g))));
                v.vertex.xyz += (o.vertexColor.r*float3(node_2160,0.0,node_2160));
...
////// Specular:
                ...
                float _Metal_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Metal );
                float3 specularColor = _Metal_var;
                float specularMonochrome;
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float3 diffuseColor = (_MainTex_var.rgb*_MainColour_var.rgb); // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : SV_Target {
                ...
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float4 _MainColour_var = UNITY_ACCESS_INSTANCED_PROP( Props, _MainColour );
                float3 diffColor = (_MainTex_var.rgb*_MainColour_var.rgb);
                float specularMonochrome;
                float3 specColor;
                float _Metal_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Metal );
                diffColor = DiffuseAndSpecularFromMetallic( diffColor, _Metal_var, specColor, specularMonochrome );
                float _Gloss_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Gloss );
